...
<?php			
	...
	function wp_statistics_log( $log_type = "" ) {
		...
		$dbname = DB_NAME;
		$result = $wpdb->query("SELECT * FROM sqlite_master WHERE type='table' AND (tbl_name = '{$wpdb->prefix}statistics_visitor' OR tbl_name = '{$wpdb->prefix}statistics_visit' OR tbl_name = '{$wpdb->prefix}statistics_exclusions' OR tbl_name = '{$wpdb->prefix}statistics_historical' OR tbl_name = '{$wpdb->prefix}statistics_pages' OR tbl_name = '{$wpdb->prefix}statistics_useronline' OR tbl_name = '{$wpdb->prefix}statistics_search')" );
